...
/**
 * What a story card has to say when you ask it.
 *
 * A card is a summary and has to stay one — a title, a rail and four
 * numbers — but the questions it raises are always the same two: what
 * is this story for, and what could I pick up right now. Both are
 * answerable from what the list already fetched, so they are answered
 * here rather than by making someone open the story to find out and
 * come back if it wasn't the one.
 *
 * Nothing here is a control. Opening the story is still the card's own
 * press, and every task named is named rather than linked, because a
 * list of small links inside a card that is itself a link is a target
